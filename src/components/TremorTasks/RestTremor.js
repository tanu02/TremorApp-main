...
export default class RestTremor extends React.Component {
  state = {
    gyroscopeData: {},
    accelerometerData: {},
    value: '',
  ...
  _unsubscribe = () => {
    ...
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/RestTremor/GyroSensor').set(this.state.list);
    var year = new Date().getFullYear();
   var week = currentWeekNumber();
firebase.database().ref('users/'+this.props.Patient+'/'+year+'/'+week+'/TremorTask/RestTremor/State').set('Done');
  };
  _unasubscribe = () => {
    this._asubscription && this._asubscription.remove();
    this._asubscription = null;
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/RestTremor/Accelerometer').set(this.state.acclist);
  };
