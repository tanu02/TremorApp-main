...
export default class NonDominantIndexMiddle extends Component {
  constructor(props) {
    super(props);
    this.state = {
      correct: 0,
  ...
  storeData(){
    ...
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/Bradykinesia/NonDominantIndexMiddle/Red/Time').set(global.myArray);
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/Bradykinesia/NonDominantIndexMiddle/Green/Time').set(global.myArra);
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/Bradykinesia/NonDominantIndexMiddle/Red/Position').set(global.XYRed);
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/Bradykinesia/NonDominantIndexMiddle/Green/Position').set(global.XYGreen);
    var year = new Date().getFullYear();
    var week = currentWeekNumber();
    firebase.database().ref('users/'+this.props.Patient+'/'+year+'/'+week+'/Bradykinesia/NonDominantIndexMiddle/State').set('Done');
    this.setState({
      buttonText: 'Redo'
    })
  ...
  render() {
    return (
      ...
      <Card>
          <Image
            style={styles.Image}
            source={require('./RapidAlternatingMovement.png')}
            />
      </Card>
