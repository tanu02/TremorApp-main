...
export default class TandemStanceBalance extends React.Component {
  state = {
    gyroscopeData: {},
    accelerometerData: {},
    value: '',
  ...
  _unsubscribe = () => {
    ...
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/BalanceTasks/TandemStanceBalance/GyroSensor').set(this.state.list);
    var year = new Date().getFullYear();
    var week = currentWeekNumber();
    firebase.database().ref('users/'+this.props.Patient+'/'+year+'/'+week+'/BalanceTasks/TandemStanceBalance/State').set('Done');
  };
  _unasubscribe = () => {
    this._asubscription && this._asubscription.remove();
    this._asubscription = null;
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/BalanceTasks/TandemStanceBalance/Accelerometer').set(this.state.acclist);
  };
  ...
  render() {
    return (
      ...
      <Card>
          <Image
            style={styles.Image}
            source={require('./TandemStanceBalance.png')}
            />
            </Card>
            <Card>
              <CardSection>
