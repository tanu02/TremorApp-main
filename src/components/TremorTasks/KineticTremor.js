...
export default class KineticTremor extends React.Component {
  state = {
    gyroscopeData: {},
    accelerometerData: {},
    value: '',
  ...
  _unsubscribe = () => {
    ...
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/KineticTremor/GyroSensor').set(this.state.list);
    var year = new Date().getFullYear();
   var week = currentWeekNumber();
firebase.database().ref('users/'+this.props.Patient+'/'+year+'/'+week+'/TremorTask/KineticTremor/State').set('Done');
  };
  _unasubscribe = () => {
    this._asubscription && this._asubscription.remove();
    this._asubscription = null;
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/KineticTremor/Accelerometer').set(this.state.acclist);
  };
  ...
  render() {
    return (
      ...
      <Card>
          <Image
            style={styles.Image}
            source={require('./KineticTremor.png')}
            />
            </Card>
            <Card>
              <CardSection>
