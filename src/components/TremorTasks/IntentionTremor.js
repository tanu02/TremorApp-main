...
export default class IntentionTremor extends React.Component {
  state = {
    gyroscopeData: {},
    accelerometerData: {},
    value: '',
  ...
  _unsubscribe = () => {
    ...
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/IntentionTremor/GyroSensor').set(this.state.list);
    var year = new Date().getFullYear();
    var week = currentWeekNumber();
    firebase.database().ref('users/'+this.props.Patient+'/'+year+'/'+week+'/TremorTask/IntentionTremor/State').set('Done');
  };
  _unasubscribe = () => {
    this._asubscription && this._asubscription.remove();
    this._asubscription = null;
    firebase.database().ref('users/'+this.props.Patient+'/'+this.props.Date+'/TremorTask/IntentionTremor/Accelerometer').set(this.state.acclist);
  };
  ...
  render() {
    return (
      ...
      <Card>
          <Image
            style={styles.Image}
            source={require('./IntentionTremor.png')}
            />
            </Card>
            <Card>
              <CardSection>
